...
public class RateLimitRule
{
    ...
    internal string PathKey => string.IsNullOrEmpty(Path) ? PathRegex: Path;
    public string Path { get; set; } = default!;
    public string PathRegex { get; set; } = default!;
    ...
    private static int ParseTime(string timeStr)
    {
        ...
        if (string.IsNullOrEmpty(match.Value))
            throw new ArgumentException("Rate limit window was not provided or was not " + "properly formatted, must be of the form ([0-9]+(s|m|d|h))");
